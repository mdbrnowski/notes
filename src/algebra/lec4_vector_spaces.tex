...
\begin{theorem}
    Każde dwie skończone bazy danej przestrzeni wektorowej są równoliczne.
\end{theorem}
\begin{proof}
...
\begin{proof}[Dowód wystarczalności]
    Załóżmy, że $V_1 + V_2$ jest sumą prostą i weźmy dowolny wektor $v \in V_1 \cap V_2$. Wówczas możemy go zapisać na dwa sposoby:
    \[ v = v + \ol{0} = \ol{0} + v. \]
    Z założenia wynika, że te dwa sposoby są identyczne, więc $v = \ol{0}$, co dowodzi, że $V_1 \cap V_2 = \{\ol{0}\}$.
\end{proof}
\begin{proof}[Dowód konieczności]
    Załóżmy, że $V_1 \cap V_2 = \{\ol{0}\}$ oraz że istnieje wektor $v \in V_1 + V_2$ taki, że
    \[ v = v_1 + v_2 = u_1 + u_2, \]
    gdzie $v_1, u_1 \in V_1$ oraz $v_2, u_2 \in V_2$. Wówczas
    \[ v_1 - u_1 = u_2 - v_2. \]
    Lewa strona tej równości należy do $V_1$, a prawa do $V_2$, więc z założenia wynika, że obie strony są równe $\ol{0}$, czyli $v_1 = u_1$ oraz $v_2 = u_2$, co dowodzi tezy.
\end{proof}
...
\end{theorem}
\begin{proof}
    Niech $B_0$ będzie bazą podprzestrzeni $V_1 \cap V_2$. Możemy rozszerzyć ją do bazy $B_1$ podprzestrzeni $V_1$ oraz do bazy $B_2$ podprzestrzeni $V_2$. Oznaczmy
    \begin{align*}
        B_0 &= \{u_1, u_2, \ldots, u_k\}, \\
        B_1 &= B_0 \cup \{v_1, v_2, \ldots, v_m\}, \\
        B_2 &= B_0 \cup \{w_1, w_2, \ldots, w_n\}
    \end{align*}
    oraz
    \[ B = B_0 \cup \{v_1, v_2, \ldots, v_m\} \cup \{w_1, w_2, \ldots, w_n\}. \]
    Łatwo sprawdzić, że $B$ generuje przestrzeń $V_1 + V_2$ oraz że jest zbiorem liniowo niezależnym, więc jest bazą tej przestrzeni. Wymiar przestrzeni $V_1 + V_2$ jest więc równy
    \begin{align*}
        |B| &= |B_0| + |B_1 \setminus B_0| + |B_2 \setminus B_0| = \\
        &= \dim(V_1 \cap V_2) + (\dim V_1 - \dim(V_1 \cap V_2)) + (\dim V_2 - \dim(V_1 \cap V_2)) = \\

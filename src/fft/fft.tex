...
\section{Reprezentacja i mnożenie wielomianów}
    ...
    \vocab{Splot} (ang.\ \textit{convolution}) ciągów współczynników dwóch wielomianów jest ciągiem współczynników ich iloczynu, więc czesto zamiast iloczynu wielomianów będziemy używali pojęcia ,,splot wielomianów'' i oznaczali
    \[ C = A * B. \]
    ...
    \end{fact}
    Na podstawie tego wzoru możemy z łatwością skonstruować algorytm mnożenia dwóch wielomianów, który będzie działać w czasie $\Theta(n^2)$. Niestety, taka złożoność nas nie zadowala; będziemy szukać algorytmu podkwadratowego $o(n^2)$.
    ...
    Możemy zauważyć, że do postaci punktowej przechodzimy z postaci wielomianowej, więc w każdym przypadku wiemy, że dany wielomian istnieje. Dla potrzeb tego artykułu wystarczy więc jedynie pokazać, że taki wielomian jest jednoznacznie określony, na co można przedstawić dowód nie wprost, który nie używa pojęć algebry liniowej.
...
\section{Dziel i zwyciężaj}
    ...
    Aby osiągnąć liniowo-logarytmiczną złożoność obliczeniową dla problemu mnożenia wielomianów, potrzebujemy już jedynie szybkiego sposobu na interpolację wielomianu dla pierwiastków jedności.
...
\begin{thebibliography}{9}
    ...
    \bibitem{ref:Smykalov} \textbf{fft: optimizations}, Vladimir Smykalov, UPS Toulouse.

...
\begin{tikzpicture}[
    node distance=1.5cm,
    problem/.style={
        rectangle, draw, minimum width=1.5cm, minimum height=.5cm, align=center,
        font=\scriptsize
    },
    label/.style={
    ...
    \node[problem, below left=1.8cm and -1cm of IS] (Clique) {\textsc{Clique}};
    \draw[reduction] (IS) -- node[label] {\ref{t:Clique}} (Clique);

    \node[problem, below right=1cm and -0.6cm of Clique] (VertexCover) {\textsc{Vertex Cover}};
    \draw[reduction] (Clique) -- node[label] {todo} (VertexCover);
    ...
    \draw[reduction] (ThreeColor) -- node[label] {todo} (Planar3Color);
    ...
    \node[problem, below=of X3C] (SubsetSum) {\textsc{Subset Sum}};
